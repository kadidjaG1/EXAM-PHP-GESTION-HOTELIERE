...
class ReservationController
{
    ...
    public function createReservation(string $nomClient, $numeroChambre, $nombreNuits, string $typeChambre): array
    {
        ...
        if (!in_array($numeroChambre, ReservationModel::ROOM_NUMBERS, true)) {
            return ['success' => false, 'message' => "Numéro de chambre invalide." ];
        }

        $availableRooms = $this->getAvailableRooms();
        if (!in_array($numeroChambre, $availableRooms, true)) {
            return ['success' => false, 'message' => "La chambre {$numeroChambre} n'est pas disponible." ];
        }
    ...
    /**
     * Retourne les chambres libres actuellement disponibles.
     */
